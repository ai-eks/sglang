    ...
    async fn execute(
        ...
        let (discovered_labels, detected_runtime) = match connection_mode {
            ConnectionMode::Http => {
                ...
                // The identity labels come from /model_info: `update_weights_from_disk`
                // moves `model_path` and `served_model_name` on the worker's manager,
                // while /server_info keeps answering the launch record — taking them
                // from there registers the worker under the model it started with, and
                // the /v1/models fallback below never fires because the stale values
                // already filled these keys. /server_info stays the fallback for a
                // worker whose /model_info predates the fields.
